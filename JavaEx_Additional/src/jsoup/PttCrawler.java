...
public class PttCrawler {
	...
	public static void main(String[] args) throws IOException {
		...
		Document doc = Jsoup.connect(GOSSPING_MAIN).cookies(cookies).get(); //Jsoup的方法在這裡
		if (doc != null) {
			...
			//get(1)取得第二個<a > element, 的href屬性, 
			//用replaceAll搭配Regular Expression取得index後面那組數字($1是指第一組小括號裡的資料)
			int newIndex = Integer.parseInt(str) + 1; 
			System.out.println(newIndex);

			// ********************* 2. 取得最新頁面文章標題 ********************* //
			String gossipingURL = "https://www.ptt.cc/bbs/Gossiping/index" + newIndex + ".html"; 
			//藉由剛剛解析最新文章頁面index取得最新文章頁面完整網址
