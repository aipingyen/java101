...
public class TestJoin {
	public static void main(String args[]) {
		...
		try {
			...
			nt1.t.join();//使用.join,確保nt1執行完才會跳回main 
			nt2.t.join();//使用.join,確保nt2執行完才會跳回main 
			nt3.t.join();//使用.join,確保nt3執行完才會跳回main 
		} catch (InterruptedException e) {
			System.out.println("Main thread Interrupted");
		}

		System.out.println("後來 Thread NT1 is alive: " + nt1.t.isAlive()); //會看到nt1~3都執行完了
